import { path, fs, type ConfigFile } from '.'

/** The location of the project's router */
export function router_path(config: ConfigFile) {
	return path.join(base_dir(config), 'Router.jsx')
}

/** The location of the page component */
export function page_entry_path(config: ConfigFile, id: string, base?: string) {
	return path.join(page_entries_dir(config, base), `${id}.jsx`)
}

export function server_adapter_path(config: ConfigFile, base?: string) {
	return path.join(units_dir(config, base), 'render', 'server.js')
}

export function adapter_config_path(config: ConfigFile, base?: string) {
	return path.join(units_dir(config, base), 'render', 'config.js')
}

export function vite_render_path(config: ConfigFile, base?: string) {
	return path.join(units_dir(config, base), 'render', 'vite.js')
}

export function app_component_path(config: ConfigFile, base?: string) {
	return path.join(units_dir(config, base), 'render', 'App.jsx')
}

export function page_unit_path(config: ConfigFile, id: string, base?: string) {
	return path.join(page_units_dir(config, base), `${id}.jsx`)
}

export function layout_unit_path(config: ConfigFile, id: string, base?: string) {
	return path.join(layout_units_dir(config, base), `${id}.jsx`)
}

export function componentField_unit_path(config: ConfigFile, id: string, base?: string) {
...
export function temp_dir(config: ConfigFile, key: string) {
	return path.join(config.rootDir, 'temp', key)
}

export function router_index_path(config: ConfigFile) {
...
export function page_entries_dir(config: ConfigFile, base?: string) {
	return path.join(units_dir(config, base), 'entries')
}

function page_units_dir(config: ConfigFile, base?: string) {
	return path.join(units_dir(config, base), 'pages')
}

function layout_units_dir(config: ConfigFile, base?: string) {
	return path.join(units_dir(config, base), 'layouts')
}

function fallbacks_units_dir(config: ConfigFile, which: 'page' | 'layout', base?: string) {
	return path.join(units_dir(config, base), 'fallbacks', which)
}

export function units_dir(config: ConfigFile, base: string = base_dir(config)) {
	return path.join(base, 'units')
}

function base_dir(config: ConfigFile) {
