...
The default cache policy as well as other parameters can be changed in your [config file](~/core/config).
...
If you set `defaultLifetime` in your [config file](~/core/config#fields) then data will
...
information, check out the [Cache API reference](~/core/cache)
.
