...
The extension activates in any workspace that contains a `houdini.config.js` or `houdini.config.ts` and needs no configuration. It is also published on [Open VSX](https://open-vsx.org/extension/houdini-graphql/houdini-graphql) for VSCodium-based editors like Cursor and Windsurf.
