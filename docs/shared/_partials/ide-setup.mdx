The [Houdini GraphQL extension](https://marketplace.visualstudio.com/items?itemName=houdinigraphql.houdini-graphql) for VS Code gives your editor the same understanding of your project that the compiler has:
...
The extension activates in any workspace that contains a `houdini.config.js` or `houdini.config.ts` and needs no configuration. It is also published on [Open VSX](https://open-vsx.org/extension/houdinigraphql/houdini-graphql) for VSCodium-based editors like Cursor and Windsurf.
