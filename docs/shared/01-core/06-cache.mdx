...
There are times where Houdini's [automatic cache updates](~/updating-data/mutations)
or [list operation fragments](~/updating-data/updating-lists#operations) are not sufficient. For those times,
...
For more information about fragment variables, head over to the [fragment api reference](~/loading-data/fragments#fragment-arguments).
...
API for the same operations supported by the [list operation fragments](~/updating-data/updating-lists#operations).
...
information on stale data in Houdini, check out the [Svelte Caching Data guide](~/core/cache#stale-data).
