...
power [its GraphQL APIs](/svelte/graphql-documents/).

## SvelteKit example

Adding it to your project is done in your vite config file. Make sure that Houdini's plugin comes before SvelteKit:
...
The plugin can be optionally configured with an object containing any of the [config values](/svelte/api/config) as well as
