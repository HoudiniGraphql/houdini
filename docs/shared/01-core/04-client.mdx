...
only sends queries and mutations over http(s) then all you need is to optionally provide
additional arguments to the fetch call as shown above. Note that the client does not take
your API's URL; that is set with the `url` value in `houdini.config.js` (see [Config](~/core/config)).
...
- `plugins` (optional): a list of [ClientPlugins](~/extending-houdini/client-plugins) that will be added to the Client's default list
...
### SvelteKit error helper example
...
- `throwOnError`: included when `throwOnError` is configured on the client.
- `fetchParams`: included when `fetchParams` is configured on the client.
...
:::tip[Defining the Full Pipeline]
