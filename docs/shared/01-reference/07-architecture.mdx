...
Houdini is built around a compiler-first model. We write GraphQL documents in our app, and Houdini reads them at build time, validates them against our schema, and generates the runtime code and TypeScript types our framework integration needs. The goal is to move as much work as possible out of the browser and into code generation.
