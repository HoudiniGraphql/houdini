---
title: Cache API
description: Programmatic access to Houdini's runtime cache for reading, writing, and subscribing to data outside of document stores.
---

## Cache API

There are times where Houdini's [automatic cache updates](/svelte/api/mutation#updating-fields)
or [list operation fragments](/svelte/api/mutation#lists) are not sufficient. For those times,
...
The imperative cache API is disabled by default. Enable it in your config file:
...
For more information about fragment variables, head over to the [fragment api reference](/svelte/api/fragment#fragment-arguments).
...
API for the same operations supported by the [list operation fragments](/svelte/api/mutation#lists).
...
information on stale data in Houdini, check out the [Svelte Caching Data guide](/svelte/cache/#stale-data).
