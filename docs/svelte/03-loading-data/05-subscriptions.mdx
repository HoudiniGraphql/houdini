...
Subscriptions can be defined like any other document and are triggered using the subscription store's `listen` method. The `$effect` re-runs automatically when `itemID` changes, creating a new subscription each time.

## List Operations

Subscription bodies can contain all of the list operations described in the [Updating Lists](~/updating-data/updating-lists) guide.

## Configuring the client

The client configuration in this section is framework-neutral unless an example imports framework-specific utilities.
