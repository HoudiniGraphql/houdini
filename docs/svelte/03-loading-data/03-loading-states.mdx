...
we're going to be building a loading screen for the Pokédex that we constructed in the [Getting Started](~/your-first-app/getting-started) guide:

The examples below build a skeleton version of the Pokédex UI while the species data is loading.
...
<script lang="ts">
    ...
    let { SpeciesInfo } = $derived(data)

    const species = $derived($SpeciesInfo.data?.species)
</script>
...
If you followed along in the [Getting Started Guide](~/your-first-app/getting-started) then you know that if we click on the
...
:::tip[What is the Shape of This Loading State?]
...
<script lang="ts">
    ...
    let { species }: { species: NavButtons_species } = $props()

    const data = $derived(fragment(species, graphql(`
        fragment NavButtons_species on Species {
            id @loading
        }
    `)))

    const pending = $derived($data.id === PendingValue)
</script>
...
:::tip[This is Getting Pretty Repetitive]
...
## Checking for Loading with `isPending`

Comparing a field to `PendingValue` is perfect when you're looking at a single value you tagged with
`@loading`, like `species.name === PendingValue` or `$data.id === PendingValue`. The value in your hand
isn't always a leaf, though. A document-level `@loading`, a `@loading(cascade: true)`, or a fragment
reference can hand you an object or a list whose _contents_ are pending rather than the `PendingValue`
symbol itself, and `value === PendingValue` is always `false` for those.

For those cases Houdini exports `isPending`. It returns `true` when the value (or anything nested inside
it) is still loading, and as a type guard it narrows the value to its resolved type on the `false` branch:

```svelte title="src/components/CityInfo.svelte&typescriptToggle=true"
<script lang="ts">
    import { fragment, graphql, isPending } from '$houdini'
    import type { CityInfo_city } from '$houdini'

    let { city }: { city: CityInfo_city } = $props()

    const data = $derived(fragment(city, graphql(`
        fragment CityInfo_city on City @loading {
            name
            mayor {
                name
            }
...
{#if isPending($data)}
    loading...
{:else}
    {$data.name}, mayor {$data.mayor.name}
{/if}
```

Here `$data` is an object whether or not it's loading, so `$data === PendingValue` would never be `true`;
`isPending($data)` walks it for you. Reach for `isPending` whenever you're checking a whole object, a list
element, or a fragment that might still be loading, and keep `=== PendingValue` for comparing a specific
scalar field.
