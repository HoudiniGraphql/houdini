...
For more information, check out the [Working with GraphQL](/svelte/graphql-documents/) guide.
...
- `errors` contains any error values that occur for a query found outside of a route component (ie, not defined in `src/routes`). If you want to use this for managing your errors, you should enable the [quietQueryError](/svelte/api/config) configuration option.
...
- `fetch` is a function that can be used to load the most recent data from your API (subject to its [Cache Policy](/svelte/cache/)). If you want to force the request to always resolve against the API, set `policy: 'NetworkOnly'`.
...
As described in the [Working with GraphQL guide](/svelte/graphql-documents/), there are 3 things that you can do to
...
blocking during navigation, check out the deep dive in [this section](/svelte/api/query#manual-loading) of the query store
docs.
...
:::tip[Loading multiple stores simultaneously]
...
:::tip[Ways to configure blocking behavior]


1. By configuration with the parameter [defaultRouteBlocking](/svelte/api/config#svelte-plugin).
1. If you have [throwOnError](/svelte/api/client#error-handling) set with operations "all" or "query", Houdini will always block.
...
applications structure multiple times, please visit the [Loading State](/svelte/loading-states/) guide.
...
according to the pagination strategy and direction. For more information about Svelte pagination in general, check
out [this guide](/svelte/pagination/).
