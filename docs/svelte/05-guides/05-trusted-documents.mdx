...
description: A guide to trusted documents in Houdini
---
...
query for every document that your client will send.

Two ways to generate this list:
1. Configuring the `persistedQueriesPath` option in your
`houdini.config.js` file. More info in the [config section](~/core/config#fields).
2. Running the `generate` command with the `--output` flag 
...
query string. The easiest way to do this is to define a [client plugin](~/extending-houdini/client-plugins).
