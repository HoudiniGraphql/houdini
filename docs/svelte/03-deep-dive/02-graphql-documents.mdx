...
description: Where to put GraphQL documents and how Houdini turns them into route queries, fragments, mutations, and stores.
---
...
```svelte title="src/routes/myProfile/+page.svelte&typescriptToggle=true"
<script lang="ts">
...
start doing more complicated things with your queries like variables or custom logic, head over to [plugin docs](/svelte/api/query#automatic-loading) to see
...
```svelte title="src/lib/components/UserAvatar.svelte&typescriptToggle=true"
<script lang="ts">
...
```ts title="src/routes/myProfile/+page.svelte"
...
#### External mutation document

```graphql title="src/lib/graphql/UpdateUser.gql"
...
store from `$houdini` as `new MyAwesomeQueryStore()`:

```graphql title="src/lib/queries/MyAwesomeQuery.gql"
...
do that, import `graphql` from `$houdini` and pass your query:
...
For more information on each store type, please visit the [api docs](/svelte/api/query/).
...
fragments, mutations, or subscriptions, you can head over to the <a href="/api" target="_blank" rel="noreferrer">api docs</a> and come
back.
...
```typescript title="src/routes/myProfile/+page.ts&typescriptToggle=true"
...
Be careful when loading multiple stores in a single load. You want to make sure that you aren't blocking one request while you wait for another. For
...
but they all behave the exact same and can be [customized the same way](/svelte/api/query#automatic-loading).
...
Another way to get a generated load for queries, is to add `@load` to the document inside of a
svelte component:

```svelte title="src/lib/UserList.svelte&typescriptToggle=true"
<script lang="ts">
    import { graphql } from '$houdini'
...
Route components that use an inline query must use a reactive
statement when defining their store:

```svelte title="src/routes/users/+page.svelte&typescriptToggle=true"
<script lang="ts">
    import { graphql } from '$houdini'
