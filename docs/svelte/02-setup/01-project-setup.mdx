...
you can add Houdini with the Svelte CLI. Just follow questions and the tool will configure everything for you (setup url, get schema, prepare project files, etc).
...
<Notice variant="info">
  This will scaffold all the required files for Houdini to work with SvelteKit. When using a remote schema, don't forget to run `npx houdini pull-schema` to download your schema definition from your API.
...
_More information about the config file can be found [here](~/core/config)._
...
_More information about the client file can be found [here](~/core/client)._
...
Finally, update the `sveltekit({ ... })` config in `vite.config.ts` to add support for the `$houdini` alias:
```javascript title="vite.config.js"
import { sveltekit } from '@sveltejs/kit/vite'
import { defineConfig } from '@sveltejs/kit/vite'
import houdini from 'houdini/vite'
...
export default defineConfig({
  plugins: [
    houdini(),
    sveltekit({
      // ... other options set for SvelteKit to work
...
<Notice variant="info">
  Older projects or projects set up with the Svelte CLI version &lt; 0.16 will have the sveltekit config in a separate `svelte.config.js` file.
...
Install the [Houdini GraphQL extension](https://marketplace.visualstudio.com/items?itemName=houdinigraphql.houdini-graphql) for VS Code to get live diagnostics, completions, hover documentation, and go-to-definition, powered by the same compiler that builds your app. See [IDE Setup](~/setup/ide-setup) for details and other editors.
