...
If you want to read more about pagination, head over to the [pagination guide](~/loading-data/pagination).
...
page and append the result of the field tagged with `@paginate` to the existing value in our cache.
The `pageInfo` object lives at `$Info.data.species.moves.pageInfo` and contains meta data about the current page.
For a more in-depth summary of what you can do with `@paginate`, you can check out the [Pagination Guide](~/loading-data/pagination).
...
If you want to read more about what Houdini can do, we recommending checking out the [Working with GraphQL](~/loading-data/queries) guide next.
