...
```svelte title="src/components/Sprite.svelte"
<script>
...
Next we have to go back to the route and put this fragment to use. Update the query inside of `src/routes/[[id]]/Info.gql` to look like:
...
1. We then updated the route's query to use the fragment we defined in the component and passed the `$Info.data.species` reference we got from the query into our `Sprite` component as the new prop.
...
```svelte title="src/components/SpeciesPreview.svelte"
<script>
