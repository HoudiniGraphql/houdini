...
To change the default `/_api` endpoint, set `apiEndpoint` in the server-only `src/server/+config.ts` (typed `ServerConfigFile`). The resolved value is injected to the client at render, so it's never bundled into `houdini.config`:
