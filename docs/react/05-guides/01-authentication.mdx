...
| `auth.url` | Base path for the endpoint that sets the cookie. Override it to match a provider's callback URL. | `/__houdini__/auth` |
...
**`allowedOrigins` entries are full session-write delegates.** Any origin we add (in `src/server/+config`) can POST to the session endpoint and set a session, so the list should only contain origins we completely trust.
