...
For completeness, let's quickly add some buttons to navigate between the different species. Copy and paste this block as the last child of the `Container` component.

```tsx title="src/routes/[[id]]/+page.tsx&typescriptToggle=true"
<Panel side="right">
    <nav>
        <a
            href={Info.species.id - 1 <= 0 ? undefined : `/${Info.species.id - 1}`}
            className={Info.species.id - 1 <= 0 ? 'disabled' : undefined}
        >
            previous
        </a>
        <a href={`/${Info.species.id + 1}`}>next</a>
    </nav>
</Panel>
```
