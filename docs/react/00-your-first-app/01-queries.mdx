...
Then copy and paste this block as the last child of the `Container` component. You will also have to add an import for `Link` from `$houdini`.
...
the route parameter corresponds to the `$id` variable in your graphql query and since its an `Int!` in your document, that `Link` requires you to pass a 
...
evokes a lot of machinery. Behind the scenes, Houdini see you have a query with a loading state and wraps the pages that use the query in a suspense boundary
...
Errors are a fact of life when building applications. You might encounter a runtime error from accessing a feild on a `null` or `undefined` value. Or maybe your API returns an error 
for a particular input. Regardless of the reason, your application's error states should be taken just as seriously as your happy path or loading states. To faciliate this, Houdini 
...
encounters an error in the initial SSR response. To customize the response you can use helpers like `notFound()` let you signal a specific status from within a page. 
