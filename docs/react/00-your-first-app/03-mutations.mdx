...
With that in place we can now define a function that will invoke the `toggleFavorite` mutation and pass it to our button. Add the `useMutation` call shown below to your component:
...
const [, toggleFavorite] = useMutation(graphql(`
...
`useMutation` returns a tuple of `[pending, mutate]`. `pending` is `true` while the server request is in flight; `mutate` is the function you call to execute the mutation. With that in place, we can now configure the button we added earlier:

```tsx title="src/routes/[[id]]/+page.tsx&typescriptToggle=true"
<button id="favorite" onClick={() => toggleFavorite({
	variables: { id: Info.species.id }
})}>
```
...
Then, open up `src/routes/[[id]]/+page.tsx`, add imports for `FavoritePreview` and `FavoritesContainer`, and some components to visualize the list:
...
<FavoritesContainer>
    {Info.favorites.length === 0 ? (
        ...
        Info.favorites.map((favorite) => (
            <FavoritePreview key={favorite.id} species={favorite} />
        ))
    )}
</FavoritesContainer>
```
...
```graphql title="src/routes/[[id]]/+page.tsx"
mutation ToggleFavorite($id: Int!) {
	toggleFavorite(id: $id) {
		species {
			id
...
`toggle` is not the only operation you can perform on a list. Houdini also supports `insert` and `remove` as well as more advanced features such as specifying conditions for these operations. For more information, check out the [Updating Lists](~/updating-data/updating-lists) guide.
