...
When a query has `@paginate`, Houdini threads a handle prop alongside the query result — `Info$handle` in our case. The handle gives you `loadNextPage`, `loadPreviousPage`, and a `pageInfo` object with metadata about the current position. For a more in-depth summary of what you can do with `@paginate`, you can check out the [Pagination Guide](~/loading-data/pagination).
...
export default function Page({ Info, Info$handle }: PageProps) {
    ...
    return (
        // ...
        <MoveDisplay move={Info.species.moves.edges[0].node} />
        // ...
    )
}
```

Houdini supports a few different approaches for pagination but we won't go too deep just yet. In our situation,
...
<div id="move-summary">
    ...
    <div id="move-controls">
        <UpButton
            disabled={!Info$handle.pageInfo.hasPreviousPage}
            onClick={async () => await Info$handle.loadPreviousPage()}
        />
        <DownButton
            disabled={!Info$handle.pageInfo.hasNextPage}
            onClick={async () => await Info$handle.loadNextPage()}
        />
    </div>
</div>
```
