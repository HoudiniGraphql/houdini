...
export function Sprite({ species, ...props }: { species: SpriteInfo }) {
...
export function SpeciesPreview({ species, number }: { species: SpeciesPreview, number: number }) {
    const preview = useFragment(species, graphql(`
...
<div id="species-evolution-chain">
    {Info.species.evolution_chain.map((form, i) => (
        <SpeciesPreview key={form.id} species={form} number={i + 1} />
    ))}
    {/* if there are less than three species in the chain, leave a placeholder behind */}
    {Array.from({ length: 3 - Info.species.evolution_chain.length }).map((_, i) => (
        <SpeciesPreviewPlaceholder
            key={i}
            number={Info.species.evolution_chain.length + i + 1}
        />
    ))}
</div>
```
