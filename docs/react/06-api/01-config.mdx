...
- `include` (optional, default: `"src/**/*.{graphql,gql,ts,js,tsx,jsx}"`): a pattern (or list of patterns) to identify source code files.
...
- `cacheBufferSize` (optional, default: `10`): The number of queries that must occur before a value is removed from the cache. For more information, see the [Cache API](/react/api/cache/).
- `defaultCachePolicy` (optional, default: `"CacheOrNetwork"`): The default cache policy to use for queries. For a list of the policies or other information see the [Cache API](/react/api/cache/).
- `defaultPartial` (optional, default: `false`): specifies whether or not the cache should always use partial data. For more information, check out the [Partial Data guide](/react/api/cache/#partial-data).
- `defaultLifetime` (optional, default: `undefined`): Specifies after how long a data goes stale in miliseconds. [Cache stale](/react/api/cache#stale).
- `defaultKeys` (optional, default: `["id"]`): A list of fields to use when computing a record's id. The default value is `['id']`. For more information see the [Cache API](/react/api/cache/#custom-ids).
- `types` (optional): an object that customizes the resolution behavior for a specific type. For more information see the [Cache API](/react/api/cache/#custom-ids).
...
- `interval` (optional, default: `2000`): Configures the schema polling behavior for the plugin. If its value is greater than `0`, the plugin will poll the set number of milliseconds. If set to `0`, the plugin will only pull the schema when you first run `dev`. If you set to `null`, the plugin will never look for schema changes. You can see use the [pull-schema command](/react/api/cli#pull-schema) to get updates.
