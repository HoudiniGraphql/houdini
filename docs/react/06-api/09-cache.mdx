...
There are times where Houdini's [automatic cache updates](/react/api/mutation#updating-fields)
or [list operation fragments](/react/api/mutation#lists) are not sufficient. For those times,
...
For more information about fragment variables, head over to the [fragment api reference](/react/api/fragment#fragment-arguments).
...
API for the same operations supported by the [list operation fragments](/react/api/mutation#lists).
...
information on stale data in Houdini, check out the [React Caching Data guide](/react/cache/#stale-data).
