...
[Config reference](/react/api/config#adding-plugins).
...
:::note[Setup phase]
`config` → `env` → `afterLoad`

Plugins use this phase to prepare Houdini before documents are discovered: update configuration, load environment variables, and validate the final config.
:::
...
Defines whether your plugin runs before or after "core" plugins.
...
:::note[Extract and parse phase]
`extensions` → `include` → `exclude` → `extractDocuments`

This phase decides which files Houdini should inspect and extracts GraphQL documents from source files before validation.
:::
...
React source extraction can use the same hook for `tsx` and `jsx` files.
...
Plugins can extend or transform the schema and add validation rules before generated files are written.
:::
...
:::note[Generate phase]
`generate` → `includeRuntime`

This phase creates generated artifacts and copies any plugin runtime files that need to ship with the application.
:::
...
in order to resolve the import, you can use the `ensureImport` utility. A React-specific `graphqlTagReturn` example needs source copy before it can be completed.
...
[client plugins](/react/api/client-plugins) that want to add a generated portion. For example,
...
[Architecture Guide](/project/architecture/#an-example).
