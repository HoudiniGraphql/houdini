...
- `--pull-schema` or `-p` pulls the latest schema before generating. Keep in mind you only need to do this if you don't already have the most up to date version of your schema in your local filesystem. If you are defining your graphql schema inside of your application or are in a monorepo, you don't need to use this flag.
- `--output` or `-o` specifies a location for the runtime generator to leave a query map for certain flavors of [persisted queries](/react/persisted-queries/).
...
set in your [config file](/react/api/config).
