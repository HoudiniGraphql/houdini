...
For more information on using these fragments, head over to the [mutation docs](/react/api/mutation#lists).
...
`@when` provides a conditional under which the [list operation](#list-operations) should be executed. It takes arguments that match the arguments of the field tagged with `@list` or `@paginate`. For more information, check out the [mutation docs](/react/api/mutation#lists).
...
`@when` provides a conditional under which the [list operation](#list-operations) should not be executed. It takes arguments that match the arguments of the field tagged with `@list` or `@paginate`. For more information, check out the [mutation docs](/react/api/mutation#lists)
...
`@cache` is used on a query document to customize the cache behavior. This includes informaton like what [cache policy](/react/api/cache/) should be used by the runtime or if you are okay with partial results from the cache. For a full list of cache policies, check out the [Cache API](/react/api/cache/) and for more information on partial responses, see the [section on partial data](/react/api/cache/#partial-data).

### `@paginate(name: String, mode: PaginateMode)`

A field marked with `@paginate` is updated with calls to the page loaders returned by the pagination function. For more information on React pagination check out the [React guide](/react/pagination/). If you pass a value for the `name` argument, the underlying list can be updated using the [operation fragments](/react/api/mutation#lists).
...
Used to define the shape of your loading state. For more information, please visit the [React loading states guide](/react/loading-states/). Setting `cascade` to `true` will implicitly add `@loading` the entire sub-selection under the marked field.
...
`@[TypeName]_delete` (ie, something like `@User_delete`) is used to delete the user with an id found in the body of a subscription or mutation. Like the operation fragments, this directive can be applied to lists or singular values. For more information see the [mutation docs](/react/api/mutation#lists).
