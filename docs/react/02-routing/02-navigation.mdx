...
For links where `to` is computed at runtime and can't be statically verified, add `suppressTypeCheck`:

```tsx
{routes.map(({ label, url }) => (
    <Link suppressTypeCheck to={url} key={url}>
        {label}
    </Link>
))}
```

This keeps all other props typed normally while accepting any string for `to`.
