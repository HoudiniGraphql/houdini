...
Where `params` fills the path, `search` fills the query string. Pass a `search` object and Houdini serializes it onto the URL:
...
The available keys and their value types are taken from the destination route's query variables, the same source as `params`, so a typo or a mistyped value is a compile error rather than a silently empty result. A list variable accepts an array and serializes as repeated keys:
...
When you need to navigate in response to something other than a click — after a form submission, inside an effect, or from a callback — call `goto` from `useLocation`:
...
`useLocation` also exposes `pathname` and `params` if you need to read the current URL — for example, to mark a link as active:
...
See [`useLocation`](~/api-reference/useLocation) for the full API.
