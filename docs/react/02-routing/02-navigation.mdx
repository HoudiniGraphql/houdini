...
If a navigation stays pending longer than a configurable threshold, Houdini shows the destination route's [`@loading` state](~/loading-data/loading-states) (its skeleton) instead of holding the old page indefinitely. Two [config](~/core/config) options control the timing:
...
[`useNavigation`](~/api-reference/useNavigation) exposes the router's in-flight navigation so your app can render its own pending UI: a global progress bar, a spinner on the clicked link, or disabling controls while a navigation settles.
