...
When you need to navigate in response to something other than a click — after a form submission, inside an effect, or from a callback — call `goto` from `useRoute().location`:
...
export function SearchForm() {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    location.goto(`/search?q=${encodeURIComponent(query as string)}`)
  }
...
`useRoute().location` also exposes `pathname`, `params`, and `search` if you need to read the current URL — for example, to mark a link as active:
...
export function NavLink({ href, label }: { href: string; label: string }) {
  const { location } = useRoute()
  return (
    <a href={href} aria-current={location.pathname === href ? 'page' : undefined}>
      {label}
    </a>
  )
}
