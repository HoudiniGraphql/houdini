...
`pending` is true from the moment a navigation starts until the destination renders its actual content—including while the destination's `@loading` state is showing—and `to` carries the destination url while pending (`null` when idle). Note that this is different from [`useRoute`](~/api-reference/useRoute), which describes the route currently rendered: during a navigation, `useRoute` keeps answering for what's on screen while `useNavigation` describes where the router is headed.
