...
**FK deferral**: FKs use `ON DELETE CASCADE`; deferral is achieved at the connection level via `PRAGMA defer_foreign_keys = ON` (set in `openDb` on the TS side and in the Go connection pragmas), so constraint checks happen at `COMMIT`, not per-statement. This is intentional; pipeline steps batch-insert rows that temporarily violate FK integrity *within a transaction*. The Go test pool enforces FKs exactly like production (`NewTestPool` runs the same connection pragmas). Deferral only stretches to the end of the current transaction, so a reference whose target is created by a *later pipeline step* can never be an FK — that's why the user-reference columns (`selection_directives.directive`, `document_directives.directive`, `document_variable_directives.directive`, `documents.type_condition`) are plain TEXT validated by validation rules instead: list delete directives (`User_delete`) are only registered during generation, and unknown names must survive extraction so validation can report them. When a step wraps work in `db.Transaction`, always check the error the commit closure reports — a deferred FK violation surfaces at COMMIT, and swallowing it silently drops every row in the transaction (this exact bug once silently dropped user documents).
