...
test('changing variables clears subscribers', function () {
	...
	expect(
		cache.internal.getRecord(cache.id('User', '1')).getSubscribers('friends(filter: "foo")')
	).toHaveLength(1)
	...
	expect(
		cache.internal.getRecord(cache.id('User', '1')).getSubscribers('friends(filter: "not-foo")')
	).toHaveLength(1)
	expect(
		cache.internal.getRecord(cache.id('User', '1')).getSubscribers('friends(filter: "foo")')
	).toHaveLength(0)
