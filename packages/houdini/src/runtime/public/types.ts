...
export type CacheTypeDef = {
	types: {
		[typeName: string]: {
			idFields: {
				// biome-ignore lint/suspicious/noExplicitAny: ID fields can be any primitive type
				[fieldName: string]: any
			}
			fields: {
				[fieldName: string]: {
					// biome-ignore lint/suspicious/noExplicitAny: Field args can be any GraphQL argument type
					args: any
					// biome-ignore lint/suspicious/noExplicitAny: Field type can be any GraphQL type
					type: any
				}
			}
			// the fragments we know about are passed as a list of pairs
			// that map the tag return type to the data shape and input
			// biome-ignore lint/suspicious/noExplicitAny: Fragment tuples can contain any type combinations
	...
	lists: {
		[listName: string]: {
			...
			types: any
			// biome-ignore lint/suspicious/noExplicitAny: List filters can be any filter structure
			filters: any
		}
	}
	...
	// biome-ignore lint/suspicious/noExplicitAny: Query tuples can contain any type combinations
	queries: [any, any, any][]
}
...
export type ListFilters<
	...
	// biome-ignore lint/suspicious/noExplicitAny: Type constraint check needs to use any
