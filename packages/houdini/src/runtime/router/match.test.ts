...
describe('route_params', () => {
	...
	testCases.forEach(({ name, pattern, url, expected }) => {
		test(name, () => {
			...
			// what we expect
			// biome-ignore lint/style/noNonNullAssertion: Test expects pattern to match
...
describe('find_match parse and match', async () => {
	...
	for (const { name, urls, expected } of table) {
		test(name, async () => {
			...
			// biome-ignore lint/suspicious/noExplicitAny: Test manifest can use any component type
			const manifest: RouterManifest<any> = {
				pages: (() => {
					// biome-ignore lint/suspicious/noExplicitAny: Test manifest can use any page structure
					const result: Record<string, any> = {}
					for (const pattern of patterns) {
						result[pattern.page_id] = {
							id: pattern.page_id,
