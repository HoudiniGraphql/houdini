...
export function unwrapType(
	...
	if (convertRuntimeScalars && config.configFile.runtimeScalars?.[type.name.value]) {
		type = config.schema.getType(config.configFile.runtimeScalars?.[type.name.value].type)
	}
