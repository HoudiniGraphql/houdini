...
// These files wrap React's jsx factories to intercept <a params={...}> and resolve
// the href, and export a JSX namespace that overrides IntrinsicElements['a'] with a
// discriminated union of all known routes.
func (p *HoudiniReact) GenerateJsxRuntime(ctx context.Context, manifest ProjectManifest) ([]string, error) {
	...
	prodContent := buildJsxRuntimeContent(routeUnion, false)
	devContent := buildJsxRuntimeContent(routeUnion, true)

	var changed []string
	for _, pair := range []struct{ path, content string }{
		{filepath.Join(houdiniDir, "jsx-runtime.ts"), prodContent},
		{filepath.Join(houdiniDir, "jsx-dev-runtime.ts"), devContent},
	} {
		existing, _ := afero.ReadFile(p.Filesystem(), pair.path)
		if string(existing) == pair.content {
			continue
		}
		if err := p.Filesystem().MkdirAll(houdiniDir, 0755); err != nil {
			return nil, err
		}
		if err := afero.WriteFile(p.Filesystem(), pair.path, []byte(pair.content), 0644); err != nil {
			return nil, err
		}
		changed = append(changed, pair.path)
	}
	return changed, nil
}
...
// buildJsxRuntimeContent generates the content for jsx-runtime.ts (dev=false) or
// jsx-dev-runtime.ts (dev=true).
func buildJsxRuntimeContent(routeUnion string, dev bool) string {
	if dev {
		return fmt.Sprintf(`// this file is generated by houdini — do not edit
