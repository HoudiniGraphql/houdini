...
// Pre-codegen fallbacks written on fresh checkout so TypeScript can resolve
// $houdini/jsx-runtime before the Go pipeline has run. GenerateJsxRuntime
// overwrites these with the real intercepting wrappers after codegen completes.
// These stubs deliberately omit resolve-href and jsx-types imports since those
// files don't exist yet; they just re-export React's own runtime transparently.
...
export default function (ctx: VitePluginContext): PluginOption {
	...
	return {
		...
		async config(userConfig, env) {
			...
			// Write stubs so TypeScript resolves $houdini/jsx-runtime on a fresh checkout
			// before the Go pipeline has run. The Go pipeline overwrites them with real content.
		...
		resolveId(id, importer) {
			...
			// Redirect react/jsx-runtime → .houdini/jsx-runtime so MDX compiled output
			// and all other callers use the route-aware wrapper without needing a per-file
			// pragma. node_modules are excluded so component libraries keep using real React.
			// The runtimeDir guard breaks the circular dep: jsx-runtime.ts itself imports
			// react/jsx-runtime and must not be redirected back to itself.
			if (importer?.includes('/node_modules/')) return null
			const runtimeDir = path.join(
				ctx.config.root_dir,
				ctx.config.config_file.runtimeDir ?? '.houdini'
			)
			if (importer?.startsWith(runtimeDir + '/') || importer?.startsWith(runtimeDir + sep)) {
				return null
			}
