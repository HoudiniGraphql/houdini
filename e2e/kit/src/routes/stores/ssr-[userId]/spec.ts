...
test.describe('SSR-[userId] Page', () => {
  ...
  test('From HOME, navigate to page (a graphql query must happen)', async ({ page }) => {
    ...
    expect(response).toBe('{"data":{"user":{"__typename":"User","id":"store-user-query:2","name":"Samuel Jackson"}}}');
  });
  ...
  test('Check refresh', async ({ page }) => {
    ...
    expect(response).toBe('{"data":{"user":{"__typename":"User","id":"store-user-query:1","name":"Bruce Willis"}}}');
