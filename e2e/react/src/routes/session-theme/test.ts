...
// Logging in first, then setting a theme, must end with BOTH the auth token and the theme —
// proving the write merged rather than replaced.
test('merge upserts a preference, keeping the logged-in session', async ({ page }) => {
	// establish a session that carries a token
	...
	// set the theme — merges { theme } in without clobbering the token
	await page.click('[data-testid="submit"]')
