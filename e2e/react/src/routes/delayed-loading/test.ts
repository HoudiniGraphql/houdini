...
// resolved name; #chrome is the layout query. Locators are deliberately unscoped: the frame
// is rendered directly (not pinned as a Suspense fallback), so exactly one #name exists at a
// time — Playwright's strict mode then doubles as a regression check that no hidden
// duplicate of the page is ever left in the DOM.
test.describe('delayed loading state', () => {
	...
	test('client nav: a fast navigation never shows the loading state', async ({ page }) => {
		...
		await expect(page.locator('#name')).not.toHaveText('loading')
		await expect(page.locator('#name')).toHaveText('Bruce Willis')
	})
	...
	test('client nav: the layout query stays live during a slow navigation', async ({ page }) => {
		await goto(page, routes.delayed_loading + '?delay=0')
		await expect(page.locator('#chrome')).toHaveText('Bruce Willis')
