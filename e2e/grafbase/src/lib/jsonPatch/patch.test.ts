...
// These test cases are copied from here: https://github.com/json-patch/json-patch-tests/blob/master/spec_tests.json
...
describe('jsonPatch', function () {
	for (const row of tests) {
		test(row.comment, function () {
			const run = () => applyPatch(row.doc, row.patch);
			if (row.error) {
				expect(run).toThrow();
			}
			expect(run()).toEqual(row.expected);
		});
	}
});
